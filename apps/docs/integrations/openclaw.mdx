...
The AI can use these tools autonomously during conversations. With custom container tags enabled, all tools support a `containerTag` parameter.
...
<AccordionGroup>
  <Accordion title="Manual Configuration">
    ...
    export SUPERMEMORY_BASE_URL="http://localhost:6767"   # optional
    ```
    ...
    | `baseUrl` | `string` | `https://api.supermemory.ai` | API endpoint (self-hosted / local). |
